...
function registeCandidate (event) {
    ...
    if (!candidate.fullName || !candidate.email || !candidate.birthDate || !candidate.biPassport || !candidate.courseId || !candidate.secretKey) {
    ...
    if(resp.success) {
        ...
        setTimeout(()=> {
            const goIndexLink = document.getElementById('index-link') 
            goIndexLink.click()
        }, 2000)
    }
